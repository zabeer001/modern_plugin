 ...
 *
 * Data entry form which allows users to create, update and test data tables. A data table consists of a database
 * table, a number of columns and some options. A shortcode can be generated for a data table. The shortcode can
...
class WPDA_Publisher_Form extends WPDA_Simple_Form {
    ...
    public function __construct(
        ...
        $args['column_headers'] = array(
            ...
            'pub_column_names'                => __( 'Column Names (* = all)', 'wp-data-access' ),
            'pub_format'                      => __( 'Column Labels', 'wp-data-access' ),
            ...
            'pub_style_user'                  => __( 'Use Custom Style', 'wp-data-access' ),
            'pub_style_color'                 => __( 'Color', 'wp-data-access' ),
            'pub_style_space'                 => __( 'Spacing', 'wp-data-access' ),
            'pub_style_corner'                => __( 'Border Radius', 'wp-data-access' ),
            ...
            'pub_responsive_cols'             => __( 'Number Of Columns', 'wp-data-access' ),
            'pub_responsive_type'             => __( 'Type', 'wp-data-access' ),
            'pub_responsive_modal_hyperlinks' => __( 'Hyperlinks On Modal', 'wp-data-access' ),
            'pub_responsive_icon'             => __( 'Show Icon', 'wp-data-access' ),
    ...
    public function show( $allow_save = true, $add_param = '' ) {
        ...
        ?>
			<style>
                ...
                #manual_styling div span {
					display: grid;
                    grid-template-rows: auto 1fr;
                }
                #manual_styling div span span {
                    display: grid;
        ...
        ?>;
				...
				function set_responsive_columns() {
					if (jQuery('#pub_responsive').val() == 'Yes') {
						...
						jQuery('#pub_flat_scrollx').parent().parent().hide();
					} else {
						...
						jQuery('#pub_flat_scrollx').parent().parent().show();
					}
				}
        ...
        ?>
				...
				function pre_submit_form() {
					...
					// Check column names
					if (jQuery("#pub_column_names").val().trim()==="") {
					...
					jQuery(dialog).dialog(
						{
							...
							title: 'Add column(s) to data table',
							modal: true,
							autoOpen: true,
							closeOnEscape: false,
							resizable: false,
    ...
    public static function test_publication() {
        ...
        if ( !wp_verify_nonce( $wp_nonce, "wpda-publication-{$pub_id}" ) ) {
            ...
            $publication = '<strong>' . __( 'ERROR: Cannot test data table [wrong arguments]', 'wp-data-access' ) . '</strong>';
        } elseif ( !$datatables_enabled || !$datatables_responsive_enabled ) {
            $publication = '<strong>' . __( 'ERROR: Cannot test data table', 'wp-data-access' ) . '</strong><br/><br/>' . __( 'SOLUTION: Load jQuery DataTables: WP Data Access > Manage Plugin > Back-End Settings', 'wp-data-access' );
        } else {
